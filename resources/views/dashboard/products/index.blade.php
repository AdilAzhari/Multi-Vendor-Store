                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-2 py-1">Store</th>
                    ...
                    <tbody>
                        @forelse ($products as $product)
                            <tr
                                ...
                                <td class="px-2 py-1">{{ $product->category ? $product->category->name : 'N/A' }}</td>
                                <td class="px-2 py-1">
                                    @if ($product->category == null)
                                        {{ 'N/A' }}
                                    @elseif ($product->category != null && $product->category->parent != null)
                                        {{ $product->category->parent->name }}
                                    @else
                                        {{ $product->category->parent }}
                                    @endif
                                </td>
                                <td class="px-2 py-1">{{ $product->store ? $product->store->name : 'N/A' }}</td>
                                <td class="px-2 py-1">{{ $product->price }}</td>
                                <td class="px-2 py-1">{{ $product->status }}</td>
                                <td class="px-2 py-1">{{ $product->created_at->diffForHumans() }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="px-2 py-1 text-center">No products found</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
